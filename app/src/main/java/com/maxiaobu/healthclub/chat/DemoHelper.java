package com.maxiaobu.healthclub.chat;
...
import com.maxiaobu.healthclub.chat.domain.RobotUser;
import com.maxiaobu.healthclub.chat.parse.UserProfileManager;
import com.maxiaobu.healthclub.chat.receiver.CallReceiver;
import com.maxiaobu.healthclub.chat.utils.PreferenceManager;
...
import maxiaobu.easeui.controller.EaseUI;
import maxiaobu.easeui.domain.EaseEmojicon;
import maxiaobu.easeui.domain.EaseEmojiconGroupEntity;
import maxiaobu.easeui.domain.EaseUser;
import maxiaobu.easeui.model.EaseAtMessageHelper;
import maxiaobu.easeui.model.EaseNotifier;
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    // TODO: 2016/9/6 全局消息接收处理  联系人昵称及头像
                    /*final String userId = message.getFrom();
                    EaseUser userInfo = getUserInfo(userId);
                    if (TextUtils.isEmpty(userInfo.getNick()) || TextUtils.isEmpty(userInfo.getAvatar())) {
                        Toast.makeText(appContext, "sadf", Toast.LENGTH_SHORT).show();
                        App.getRequestInstance().post(App.getInstance(), UrlPath.URL_ACCOUNT_INFO, new RequestParams("memid", userId), new RequestListener() {
                            @Override
                            public void requestSuccess(String s) {
                                ...
                                // 存入内存
                                getContactList();
                                contactList.put(userId, easeUser);
                                // 存入db
